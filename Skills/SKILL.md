...
- **Prefer the /v1/ project endpoint for all fine-tuning operations** (file upload, job submission, monitoring). It supports all features including Python graders without needing to track API version strings. Use `openai.OpenAI(base_url="https://<resource>.services.ai.azure.com/api/projects/<project>/openai/v1/", api_key=KEY)`. The versioned Azure endpoint (`?api-version=YYYY-MM-DD`) may lag behind on feature support.
...
- **DPO hyperparameters go inside method config**: When submitting DPO jobs, `n_epochs`, `learning_rate_multiplier`, and `beta` must be inside `method.dpo.hyperparameters`, NOT at the top-level `hyperparameters` field. Top-level HPs cause `invalidPayload` error.
