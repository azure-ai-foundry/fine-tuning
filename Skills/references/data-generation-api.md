...
| `Something went wrong during data generation. Please try again.` (SUCCESS → FAILED in ~15-20s) | Generic, multiple causes: teacher model can't serve the request (wrong model, missing permissions, FT-only deployment); or this project rejects inline `Prompt` source for `SimpleQnA + SFT` even though it accepts the same source for `EVALUATION` (observed on `REDACTED-FOUNDRY-PROJECT`). | First, try a different `--teacher` (e.g. `gpt-4.1-mini`, `gpt-4o`). If the failure repeats with the `Prompt` source, switch to `--source prompt-file` (which uploads the prompt as `user_data` and uses a `File` source) or `--source file`. File sources tend to work even when the same content fails as a `Prompt`. Confirm the deployment exists and is healthy. |
