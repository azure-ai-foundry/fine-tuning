...
> **Status:** The matrix below has been verified end-to-end:
> - SDK + REST end-to-end with `SimpleQnA` + `File` source + `SUPERVISED_FINETUNING`
> - SDK with `SimpleQnA` + `Prompt` source + `EVALUATION` (Dataset output)
> - SDK with `SimpleQnA` + `Agent` source + `EVALUATION` (Dataset output)
> - SDK with `Traces` + `traces` recipe + `SUPERVISED_FINETUNING`
...
>
> **Not yet validated end-to-end:** `REINFORCEMENT_FINETUNING` + `Traces` requires a trace stream with ground-truth assistant answers; `Agent` source + `ToolUseFineTuning` requires an agent whose tool definitions are registered server-side (hosted agents that surface tools only at runtime are rejected — see constraints below).
...
**Observed runtimes** (gpt-4.1-mini teacher):
...
| `Something went wrong during data generation. Please try again.` (SUCCESS → FAILED in ~15-20s) | Generic, multiple causes: teacher model can't serve the request (wrong model, missing permissions, FT-only deployment); or this project rejects inline `Prompt` source for `SimpleQnA + SFT` even though it accepts the same source for `EVALUATION`. | First, try a different `--teacher` (e.g. `gpt-4.1-mini`, `gpt-4o`). If the failure repeats with the `Prompt` source, switch to `--source prompt-file` (which uploads the prompt as `user_data` and uses a `File` source) or `--source file`. File sources tend to work even when the same content fails as a `Prompt`. Confirm the deployment exists and is healthy. |
